  ...
  return createPortal(
    ...
    >
      <button
        ...
        onClick={onClose}
        aria-label="Close"
        className="absolute top-4 right-4 z-10 p-1.5 rounded-md bg-white/10 text-white hover:bg-white/20 active:scale-90 transition-all"
      >
      ...
      <div className="max-w-full max-h-full animate-materialize" onClick={(e) => e.stopPropagation()}>
        ...
        >
          <img
            ...
            alt={alt}
            draggable={false}
            className="max-w-full max-h-[85vh] object-contain rounded-md select-none pointer-events-none"
          />
          {caption && (
