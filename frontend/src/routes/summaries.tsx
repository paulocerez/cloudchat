      ...
      <div className="mt-2 h-4 flex items-center">
        {isPending ? (
          <span className="flex items-center gap-1.5 text-xs text-gray-400">
            <span className="w-3 h-3 rounded-full border-2 border-gray-300 border-t-gray-600 animate-spin-slow" />
            Generating…
          </span>
        ) : done ? (
...
function SummaryCard({ summary }: { summary: AISummary }) {
  ...
  return (
    <Link
      ...
      className="flex items-start justify-between gap-4 py-4 -mx-3 px-3 rounded-lg transition-all duration-200 hover:bg-gray-50 group"
    >
