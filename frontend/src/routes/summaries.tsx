          ...
          <div className="divide-y divide-line">
            {[1, 2].map((i) => (
              <div key={i} className="py-4 animate-pulse">
                <div className="h-2.5 bg-sunken rounded w-20 mb-2" />
...
function EarlierWeeks() {
  ...
  return (
    <div className="mt-3 flex items-center gap-2 rounded-xl bg-surface border border-line p-3">
      ...
      <button
        ...
        className="shrink-0 text-xs font-medium rounded-lg px-3 py-1.5 bg-ink text-on-ink transition-all duration-150 hover:bg-ink-hover active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-ink"
      >
        {isPending ? (
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full border-2 border-on-ink/40 border-t-on-ink animate-spin-slow" />
            Generating…
          </span>
        ) : isSuccess ? (
      ...
      <div className="mt-2 h-4 flex items-center">
        {isPending ? (
          <span className="flex items-center gap-1.5 text-xs text-faint">
            <span className="w-3 h-3 rounded-full border-2 border-line-strong border-t-secondary animate-spin-slow" />
            Generating…
          </span>
        ) : done ? (
...
function SummaryCard({ summary }: { summary: AISummary }) {
  ...
  return (
    <Link
      ...
      className="flex items-start justify-between gap-4 py-4 -mx-3 px-3 rounded-lg transition-all duration-200 hover:bg-hover group"
    >
      <span className="mt-1 w-0.5 h-12 rounded-full bg-line-strong shrink-0 transition-colors duration-200 group-hover:bg-muted" />
